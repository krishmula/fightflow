...
from models.advanced_cnn import pipeline as pipeline_impl

def load_custom_model(checkpoint_path: Path, device: torch.device):
    checkpoint = pipeline_impl.load_checkpoint(checkpoint_path, device)
    class_names = checkpoint.get("class_names", ["jab", "negative"])
    image_size = checkpoint.get("image_size", 128)
    
    model = pipeline_impl.AdvancedCNN(num_classes=len(class_names)).to(device)
    model.load_state_dict(checkpoint["model_state_dict"])
    model.eval()
    
    _, eval_tf = pipeline_impl.get_transforms(image_size)
    return model, eval_tf, class_names
...
def main(args) -> None:
    ...
    checkpoint_path = pipeline_impl.resolve_project_path(args.checkpoint)
    model, preprocess, labels = load_custom_model(checkpoint_path, device)
