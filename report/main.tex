...
and the Baseline CNN (0.402). The CNN+LSTM reaches a macro-F1 of 0.588,
...
\subsection{Data Leakage and Correction}
Early experiments used clip-level random splits, where clips from the same
video could appear in both the training and test sets. Under this setup,
models achieved inflated macro-F1 scores of up to 0.933 for ResNet-18 and
0.877 for the Advanced CNN. These results were not reproducible on unseen
videos and indicated that models were memorizing video-specific visual
patterns rather than learning generalizable punch signatures. We identified
this as data leakage and corrected it by adopting the video-level GSGS split
described above. All results reported in this paper are from the
...
Notably, the CNN+LSTM also peaks at epoch~7, the same unfreeze epoch as
ResNet-18. This suggests that the marginal improvement of CNN+LSTM over
ResNet-18 (0.588 vs.\ 0.570) may stem from the backbone receiving additional
gradient signal through the LSTM loss rather than from the LSTM learning
genuine temporal structure. Isolating the LSTM's true contribution would
require a frozen-backbone ablation where only the LSTM is trained.
...
Biomechanically, a hook and a straight differ in one precise way: in a
straight punch, the elbow angle opens from approximately 90 degrees at the
cocked position to near 180 degrees at full extension, while in a hook the
elbow angle remains close to 90 degrees throughout as the arm sweeps
laterally as a near-rigid unit. This means the rate of change of elbow angle
over the clip is a direct discriminator between the two classes. The current
feature set includes static elbow angles per frame but does not explicitly
include elbow angle velocity (the per-frame delta of the elbow angle). Adding
this 2-dimensional feature (one per arm) would directly encode the signal that
separates hooks from straights and is a targeted fix for future work. The two
main confusion patterns in the Pose-LSTM -- 52 straight samples predicted as
hook and 55 none samples predicted as hook -- are consistent with this missing
temporal derivative: without it, hooks and straights look similar in their
static pose, and the low-velocity guard position of \textit{none} overlaps
with the held elbow angle of a hook.
...
(0.570) and a CNN+LSTM temporal model (0.588). The training dynamics in
...
multiple fighters and environments; (2) adding elbow angle velocity as an
explicit feature to directly encode the hook-vs-straight discriminator
identified in Section~\ref{sec:discussion}; (3) applying GCN-based
architectures over the MediaPipe pose graph; (4) integrating optical flow as
an additional modality; and (5) deploying the Pose-LSTM in a real-time edge
coaching system.
